-- Run this script to erase all entries in the database, and reload the look up tables.
-- This file is not part of the installation of the O&M database. 
-- It's purpose is to quickly restart the database when trying the python function to insert data.
-- It doesn't delete any of the stored procedures so no need to resintall them.

-- Restart database trials
SELECT citydb_pkg.unom5_cleanup_schema('citydb');
